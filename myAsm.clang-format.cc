...
enum struct ExOpCode { x0 = 0, x1, x2, x3, x4, x5, x6, x7 };

enum struct Mode { ind = 0, ind8, ind32, dir };
...
// z: word or dword
enum struct BasicOperands { EbGb = 0, EvGv, GbEb, GvEv, ralIb, raxIz };

enum struct Scale { x1 = 0, x2, x4, x8 };

uint8_t makeRex(bool x64, Register destination, Register index,
                Register source) {
...
uint8_t makeModRxRm(Mode mode, uint8_t rx, Register source) {
...
uint8_t makeModRxRm(Mode mode, ExOpCode xop, Register source) {
  auto x = static_cast<uint8_t>(xop);

  return makeModRxRm(mode, x, source);
}

uint8_t makeModRxRm(Mode mode, Register destination, Register source) {
  auto x = static_cast<uint8_t>(destination);

  return makeModRxRm(mode, x, source);
}

uint8_t makeModRxRm(uint8_t rx, Register source) {
...
uint8_t makeModRxRm(Register destination, Register source) {
  return makeModRxRm(static_cast<uint8_t>(destination), source);
}

uint8_t makeModRxRm(ExOpCode x, Register r) {
  return makeModRxRm(static_cast<uint8_t>(x), r);
}

uint8_t makeSIB(Scale scale, Register index, Register base) {
...
size_t useIndr4r12(uint8_t x, Register r2, Code &where) {
...
size_t useIndr4r12(Register r1, Register r2, Code &where) {
  return useIndr4r12(static_cast<uint8_t>(r1), r2, where);
}

size_t useIndr4r12(ExOpCode x, Register r2, Code &where) {
  return useIndr4r12(static_cast<uint8_t>(x), r2, where);
}

size_t useIndr5r13(uint8_t x, Register r2, Code &where) {
...
size_t useIndr5r13(Register r1, Register r2, Code &where) {
  return useIndr5r13(static_cast<uint8_t>(r1), r2, where);
}

size_t useIndr5r13(ExOpCode x, Register r2, Code &where) {
  return useIndr5r13(static_cast<uint8_t>(x), r2, where);
}

size_t makeIndirect(uint8_t x, Register reg, Code &where) {
  size_t length = 0;
...
size_t makeIndirect(Register destination, Register source, Code &where) {
  return makeIndirect(static_cast<uint8_t>(destination), source, where);
}

size_t makeIndirect(ExOpCode xop, Register source, Code &where) {
  return makeIndirect(static_cast<uint8_t>(xop), source, where);
}

size_t makeBasicIns(BasicOpClass op, Register destination, Register source,
                    Code &where) {
...
size_t makeImm32(int32_t imm, Code &where) {
...
size_t makeBasicIns(BasicOpClass op, int32_t imm32, Code &where) {
  auto o = static_cast<uint8_t>(op);
...
size_t makeBasicIns(BasicOpClass op, Register destination, int32_t imm32,
                    Code &where) {
...
size_t makeBasicIns(BasicOpClass op, IndirectReg destination, int32_t imm32,
                    Code &where) {
...
size_t makeBasicIns(BasicOpClass op, IndirectReg destination, Register source,
                    Code &where) {
  auto dest = static_cast<Register>(destination);
  auto o = static_cast<uint8_t>(op);
...
// destination = destination op [source]
size_t makeBasicIns(BasicOpClass op, Register destination, IndirectReg source,
                    Code &where) {
...
size_t makeMul(Register source, Code &where) {
...
size_t makeMul(IndirectReg source, Code &where) {
  auto src = static_cast<Register>(source);
...
size_t makeMul(Register destination, Register source, Code &where) {
  if (destination == Register::rax) {
...
size_t makeMul(Register destination, IndirectReg source, Code &where) {
  auto src = static_cast<Register>(source);
...
size_t makeMul(Register destination, Register source, int32_t imm,
               Code &where) {
...
size_t makeMul(Register destination, IndirectReg source, int32_t imm,
               Code &where) {
  auto src = static_cast<Register>(source);
...
size_t makeDiv(Register source, Code &where) {
...
size_t makeDiv(IndirectReg source, Code &where) {
  auto src = static_cast<Register>(source);
...
size_t makeJcc(CondTest test, uint32_t disp, Code &where) {
...
size_t makeJmp(uint32_t disp, Code &where) {
...
size_t makeJmp(Register r, Code &where) {
  size_t length = 0;
...
uint8_t combineOpReg(uint8_t op, Register reg) {
  auto r = static_cast<uint8_t>(reg) & 7;

  return op | r;
}

size_t makeMov(Register destination, Register source, Code &where) {
...
size_t makeMov(Register destination, IndirectReg source, Code &where) {
  auto src = static_cast<Register>(source);
...
size_t makeMov(IndirectReg destination, Register source, Code &where) {
  auto dest = static_cast<Register>(destination);
...
size_t makeMov(Register destination, int64_t imm, Code &where) {
...
size_t makeMov(IndirectReg destination, int32_t imm, Code &where) {
  auto dest = static_cast<Register>(destination);
...
size_t makeCall(int32_t disp, Code &where) {
...
size_t makeCall(Register r, Code &where) {
  size_t i = 0;
...
size_t makeRet(Code &where) {
  where.push_back(0xc3);
  return 1;
}

// shl or shr by one
size_t makeShift(ShiftOp op, Register reg, Code &where) {
  auto xop = static_cast<ExOpCode>(op);
...
size_t makeShift(ShiftOp op, Register reg, uint8_t imm8, Code &where) {
  if (imm8 == 1) {
...
// shl or shr by one
size_t makeShift(ShiftOp op, IndirectReg reg, Code &where) {
...
size_t makeShift(ShiftOp op, IndirectReg reg, uint8_t imm8, Code &where) {
  if (imm8 == 1) {
    return makeShift(op, reg, where);
  }
...
size_t makeCompl(ExOpCode op, Register reg, Code &where) {
...
size_t makeCompl(ComplOp op, Register reg, Code &where) {
  return makeCompl(static_cast<ExOpCode>(op), reg, where);
}

size_t makeCompl(ExOpCode op, IndirectReg reg, Code &where) {
  auto r = static_cast<Register>(reg);
...
size_t makeCompl(ComplOp op, IndirectReg reg, Code &where) {
  return makeCompl(static_cast<ExOpCode>(op), reg, where);
}

size_t makeSysCall(Code &where) {
...
size_t makePush(Register reg, Code &where) {
  size_t length = 0;
...
size_t makePush(IndirectReg reg, Code &where) {
  auto r = static_cast<Register>(reg);
  size_t length = 0;
...
size_t makePush(uint32_t imm, Code &where) {
...
size_t makePop(Register reg, Code &where) {
  size_t length = 0;
...
size_t makePop(IndirectReg reg, Code &where) {
  auto r = static_cast<Register>(reg);
  size_t length = 0;
...
size_t makeIDec(IDecOp op, Register reg, Code &where) {
  auto xop = static_cast<ExOpCode>(op);
...
size_t makeIDec(IDecOp op, IndirectReg reg, Code &where) {
  auto r = static_cast<Register>(reg);
  auto xop = static_cast<ExOpCode>(op);
...
size_t makeMovS(Code &where) {
  where.push_back(0xa4);
  return 1;
}

size_t makeRep(Code &where) {
  where.push_back(0xf3);
  return 1;
}

size_t makeLoop(uint8_t disp, Code &where) {
  where.push_back(0xe2);
  where.push_back(disp);
  return 2;
}

size_t makeLoopE(uint8_t disp, Code &where) {
  where.push_back(0xe1);
  where.push_back(disp);
  return 2;
}

size_t makeLoopNE(uint8_t disp, Code &where) {
...
size_t makeSDInsPrefix(XmmReg destination, XmmReg source, XmmOp op, Code &where,
                       bool x64 = false) {
...
size_t makeSDIns(XmmReg destination, XmmReg source, XmmOp op, Code &where,
                 bool x64 = false) {
  auto d = static_cast<Register>(destination);
  auto s = static_cast<Register>(source);
  ;
...
size_t makeSDIns(XmmReg destination, IndirectReg source, XmmOp op, Code &where,
                 bool x64 = false) {
...
size_t makeMovSD(XmmReg destination, XmmReg source, Code &where) {
  return makeSDIns(destination, source, XmmOp::mov, where);
}

size_t makeMovSD(XmmReg destination, IndirectReg source, Code &where) {
  return makeSDIns(destination, source, XmmOp::mov, where);
}

// addsd
size_t makeAddSD(XmmReg destination, XmmReg source, Code &where) {
  return makeSDIns(destination, source, XmmOp::add, where);
}

size_t makeAddSD(XmmReg destination, IndirectReg source, Code &where) {
  return makeSDIns(destination, source, XmmOp::add, where);
}

// subsd
size_t makeSubSD(XmmReg destination, XmmReg source, Code &where) {
  return makeSDIns(destination, source, XmmOp::sub, where);
}

size_t makeSubSD(XmmReg destination, IndirectReg source, Code &where) {
  return makeSDIns(destination, source, XmmOp::sub, where);
}

// mulsd
size_t makeMulSD(XmmReg destination, XmmReg source, Code &where) {
  return makeSDIns(destination, source, XmmOp::mul, where);
}

size_t makeMulSD(XmmReg destination, IndirectReg source, Code &where) {
  return makeSDIns(destination, source, XmmOp::mul, where);
}

// divsd
size_t makeDivSD(XmmReg destination, XmmReg source, Code &where) {
  return makeSDIns(destination, source, XmmOp::div, where);
}

size_t makeDivSD(XmmReg destination, IndirectReg source, Code &where) {
  return makeSDIns(destination, source, XmmOp::div, where);
}

// sqrtsd square root of scalar double-precision float
size_t makeSqrtSD(XmmReg destination, XmmReg source, Code &where) {
  return makeSDIns(destination, source, XmmOp::sqrt, where);
}

size_t makeSqrtSD(XmmReg destination, IndirectReg source, Code &where) {
  return makeSDIns(destination, source, XmmOp::sqrt, where);
}

// maxsd return the larger of 2 double-precision values
size_t makeMaxSD(XmmReg destination, XmmReg source, Code &where) {
  return makeSDIns(destination, source, XmmOp::max, where);
}

size_t makeMaxSD(XmmReg destination, IndirectReg source, Code &where) {
  return makeSDIns(destination, source, XmmOp::max, where);
}

// minsd return the smaller of 2 double-precision values
size_t makeMinSD(XmmReg destination, XmmReg source, Code &where) {
  return makeSDIns(destination, source, XmmOp::min, where);
}

size_t makeMinSD(XmmReg destination, IndirectReg source, Code &where) {
  return makeSDIns(destination, source, XmmOp::min, where);
}

// cmpsd compare double-precision values with op determined by imm value and
// store
//       true of false in destination register. Does not set EFLAGS
size_t makeCmpSD(XmmReg destination, XmmReg source, SDcmp op, Code &where) {
...
size_t makeCmpSD(XmmReg destination, IndirectReg source, SDcmp op,
                 Code &where) {
  auto i = makeSDIns(destination, source, XmmOp::cmp, where);
  where.push_back(static_cast<uint8_t>(op));
...
size_t makeComiSDprefix(XmmReg destination, XmmReg source, Code &where) {
  auto s = static_cast<Register>(source);
  auto d = static_cast<Register>(destination);
  size_t c = 0;
...
size_t makeComiSD(XmmReg destination, XmmReg source, Code &where) {
  auto d = static_cast<Register>(destination);
  auto s = static_cast<Register>(source);
...
size_t makeComiSD(XmmReg destination, IndirectReg source, Code &where) {
  auto xmmS = static_cast<XmmReg>(source);
  auto d = static_cast<Register>(destination);
  auto s = static_cast<Register>(source);
...
// cvtsi2sd convert an interger general purpose regisger to a double-precision
// value in
//          an xmm register
size_t makeCvtSi2Sd(XmmReg destination, Register source, Code &where) {
  auto s = static_cast<XmmReg>(source);
  return makeSDIns(destination, s, XmmOp::cvtsi2sd, where, true);
}

size_t makeCvtSi2Sd(XmmReg destination, IndirectReg source, Code &where) {
  return makeSDIns(destination, source, XmmOp::cvtsi2sd, where);
}

// cvtsd2si convert a double precision value in an xmm register to an interger
// in a
//          general purpose register
size_t makeCvtSd2Si(Register destination, XmmReg source, Code &where) {
  auto d = static_cast<XmmReg>(destination);
  return makeSDIns(d, source, XmmOp::cvtsd2si, where, true);
}

size_t makeCvtSd2Si(Register destination, IndirectReg source, Code &where) {
  auto d = static_cast<XmmReg>(destination);
  return makeSDIns(d, source, XmmOp::cvtsd2si, where, true);
}

vector<vector<uint8_t>> nopTable = {
    vector<uint8_t>{},
    vector<uint8_t>{0x90},
...
size_t makeNop(size_t length, Code &where) {
  auto left = length;
  auto nopTableSize = nopTable.size() - 1;

  while (nopTableSize < left) {
    where.insert(where.end(), nopTable[nopTableSize].begin(),
                 nopTable[nopTableSize].end());
    left -= nopTableSize;
  }

  if (0 < left) {
    where.insert(where.end(), nopTable[left].begin(), nopTable[left].end());
  }
...
int main(int, char **) {
  ...
  // g++ -o myasm myAsm.cc ; ./myasm ;  objdump -M intel -m i386:x86-64 -b
  // binary -D test.bin > test.asm
...
#define ENCODING_TEST
  ...
  cout << "And the answer to life, the universe, and everyting is " << fn()
       << endl;
#endif
  ...
  //     ); makeBasicIns( BasicOpClass::_sub, Register( d ), IndirectReg( s ),
  //     code ); makeMul( Register( s ), code ); makeMul( IndirectReg( s ), code
  //     ); makeMul( Register( d ), Register( s ), code ); makeMul( Register( d
  //     ), Register( s ), 0x76543210, code ); makeMul( Register( d ),
  //     IndirectReg( s ), code ); makeMul( Register( d ), IndirectReg( s ),
  //     0x76543210, code ); makeDiv( Register( s ), code ); makeDiv(
  //     IndirectReg( s ), code ); makeJcc( CondTest::GE, 0x76543210, code );
